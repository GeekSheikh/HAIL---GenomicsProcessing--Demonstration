...
#/user/daniel.tomes/choa/data/gnomad/sample/vds/gnomad.genomes.r2.0.1.sites.X.vds
...
vds = hc.read("/user/tomesd/1kg.vds")
vds_result = vds.annotate_samples_table('/user/keebler/GM22624_gts.vds/testAnnotes.tsv', 'Sample', root='sa.three', config=conf)
vds_result.write('/user/tomesd/1kg_processed.vds', parquet_genotypes=True)

import json
with open('metadata_two.json') as data_file:
    data = json.load(data_file)
json.JSONDecoder().decode(json.dumps(data))
ds2 = sc.parallelizedata
ds2 = json.dumps(data)
ds2 = json.dumps(data)
df = spark.read.json(sc.parallelize(json.JSONDecoder().decode(json.dumps(data, separators=(',',':')))))
df.show()
df.printSchema

df = spark.read.json('metadata_two_mini_manual.json')
